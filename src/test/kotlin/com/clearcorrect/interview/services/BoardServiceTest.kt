...
import com.sun.javafx.scene.control.skin.ScrollBarSkin.DEFAULT_WIDTH
...
class BoardServiceTest {
    ...
    private val board = MutableList(DEFAULT_WIDTH) { MutableList(DEFAULT_WIDTH) { '.' } }
    ...
    fun setUp() {
        ...
        `when`(mockBoardTransformer.present(anyList())).thenReturn(readableBoard)
    }

    @Test
    fun create_callsBoardTransformer() {
        subject.create()
        argumentCaptor<List<List<Char>>>().apply {
            verify(mockBoardTransformer).toDBString(capture())
            then(firstValue.size).isEqualTo(15)

            firstValue.forEach { row: List<Char> ->
